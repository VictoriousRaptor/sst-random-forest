...
def main():
    ...
    for i in range(args.runs):
        ...
        dev_features = np.load('dev_features.npy')
        test_features = np.load('test_features.npy')
        train_labels = np.load('train_labels.npy')
        dev_labels = np.load('dev_labels.npy')
        test_labels = np.load('test_labels.npy')

        # clf = RandomForestClassifier(n_estimators=20, max_depth=10, n_jobs=-1, criterion='gini', max_features='log2')
        # clf.fit(np.vstack((train_features, dev_features)), np.hstack((train_labels, dev_labels)))
        # train_acc = clf.score(np.vstack((train_features, dev_features)), np.hstack((train_labels, dev_labels)))
        ...
        print(end)
        train_acc = forest.score(np.vstack((train_features, dev_features)), np.hstack((train_labels, dev_labels)))
